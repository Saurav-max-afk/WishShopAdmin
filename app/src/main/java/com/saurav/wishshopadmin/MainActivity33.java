...
public class MainActivity33 extends AppCompatActivity {
    ...
    public void uploadImage(){
        StorageReference storageReference= FirebaseStorage.getInstance()
                .getReference().child("RedmiImage").child(uri.getLastPathSegment());
        ...
        storageReference.putFile(uri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            ...
            public void onFailure(@NonNull Exception e) {
                progressDialog.dismiss();
                Toast.makeText(MainActivity33.this,e.getMessage().toString(), Toast.LENGTH_SHORT).show();
            }
        });
    ...
    public void uploadItemData(){
        ...
        FirebaseDatabase.getInstance().getReference("Redmi_Name")
                .child(myCurrentDateTime).setValue(main2Modal).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful())
                {
                    Toast.makeText(MainActivity33.this, "Item Uploaded", Toast.LENGTH_SHORT).show();
                    finish();
                }
            }
        }).addOnFailureListener(new OnFailureListener() {
